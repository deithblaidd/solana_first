...
async fn cross_programm_invocation_validator_test() {
    //change variables to deployed programm ids
    let program_caller_id =
        Pubkey::from_str("7KCL5aw2p1JYTYtHkwzn3yFFtJTKtYuZeP1Y3gn8GNZh").unwrap();
    let program_called_id =
        Pubkey::from_str("DnoWxfPYHEMGhHH8jqyE4WuMDHVaAa3dnwrcVpaoeiA4").unwrap();
    ...
    let instruction = Instruction::new_with_bytes(
        program_caller_id,
        &ix_data,
        vec![AccountMeta::new_readonly(program_called_id, false)],
    );
    ...
    let confirmed = loop {
        match client.get_transaction_with_config(
            ...
            Err(err) => {
                if err.to_string().contains("Transaction not found")
    ...
    let called_invoke = logs
        .iter()
        .any(|l| l.contains("Caller succesfully invoked called"));
